classdef NotificationDispatcherTest < matlab.unittest.TestCase
    ...
    methods (Test)
        ...
        function routesOnceByCentralPolicy(testCase)
            ...
            expectedCounts = [1, 1, 1, 0, 1];

            for sinkIndex = 1:numel(sinks)
                testCase.verifyEqual( ...
                    sinks{sinkIndex}.count(), ...
                    expectedCounts(sinkIndex), ...
                    names(sinkIndex));
            end

        end

        function warningsAndErrorsUseUiLogWithoutUiAlert(testCase)
            ...
            for level = ["warning", "error"]
                message = openmebius.core.notification.Message( ...
                    "Check the input.", ...
                    level, ...
                    EventId = "message-only-" + level, ...
                    Attention = "action-required");

                testCase.verifyTrue( ...
                    policy.shouldRoute(message, "ui-log"), level);
                testCase.verifyFalse( ...
                    policy.shouldRoute(message, "ui-alert"), level);
            end

        end

        function deployedDesktopAvoidsConsoleRouting(testCase)

            policy = openmebius.infrastructure.notification ...
                .RoutingPolicy( ...
                Mode = "desktop", ...
                ConsoleEnabled = false);
            warningMessage = openmebius.core.notification.Message( ...
                "Check the input.", ...
                "warning", ...
                EventId = "deployed-warning");

            testCase.verifyFalse( ...
                policy.shouldRoute(warningMessage, "console"));
            testCase.verifyTrue( ...
                policy.shouldRoute(warningMessage, "ui-log"));
            testCase.verifyTrue( ...
                policy.shouldRoute(warningMessage, "file"));
        ...
        function debugMessageIsHiddenFromNormalUiLog(testCase)
            ...
            dispatcher.publish(openmebius.core.notification.Message( ...
                "Hit-and-Run: saved 10/3000", ...
                "debug", ...
                EventId = "hit-and-run-progress"));

            testCase.verifyEqual(fileSink.count(), 1);
            testCase.verifyEqual(logSink.count(), 0);
