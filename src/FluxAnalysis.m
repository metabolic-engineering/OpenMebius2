classdef FluxAnalysis < openmebius.infrastructure.logging.MessageState
    ...
    methods (Access = private)
        ...
        function initializeRunMetadata(obj)
            ...
            metadata = buildAnalysisMetadata(obj);
            obj.AnalysisMetadata = metadata;
            [isSuccess, msg] = obj.Hdf5ResultRepository.writeAnalysisMetadata( ...
                obj.HDF5FilePath, ...
                metadata);
            ...
            if ~isSuccess
                ...
                notifyGeneralMessage( ...
                    obj, ...
                    "error", ...
                    "Failed to write analysis metadata: " + msg, ...
                    dbstack());
                return
            end

            [isSuccess, msg] = obj.ResultManifestRepository.writeStarted( ...
                obj.ResultLocation, ...
                metadata);
            ...
            if ~isSuccess
                ...
                notifyGeneralMessage( ...
                    obj, ...
                    "error", ...
                    "Failed to write result manifest: " + msg, ...
                    dbstack());
                return
            end
        ...
        function finalizeRunMetadata(obj)
            ...
            finishedAtUtc = string(datetime( ...
                ...
                "TimeZone", "UTC", ...
                "Format", "yyyy-MM-dd'T'HH:mm:ss.SSSXXX"));
            [isSuccess, msg] = obj.Hdf5ResultRepository.writeRunCompletion( ...
                obj.HDF5FilePath, ...
                finishedAtUtc, ...
                obj.isError, ...
                obj.isCanceled);
            ...
            if ~isSuccess
                ...
                notifyGeneralMessage( ...
                    obj, ...
                    "error", ...
                    "Failed to finalize analysis metadata: " + msg, ...
                    dbstack());
            end
        ...
        function metadata = buildAnalysisMetadata(obj)

            provenance = obj.Provenance;
            experimentNames = obj.provenanceValue( ...
                provenance, ...
                'experimentNames', ...
                string(obj.expsList(:)));
            experimentFileNames = obj.provenanceValue( ...
                provenance, ...
                'experimentFileNames', ...
                strings(size(experimentNames)));
            experimentHashes = obj.provenanceValue( ...
                provenance, ...
                'experimentSha256', ...
                strings(size(experimentNames)));
            modelFileName = obj.provenanceValue( ...
                provenance, ...
                'modelFileName', ...
                "");
            modelHash = obj.provenanceValue( ...
                provenance, ...
                'modelSha256', ...
                "");

            if strlength(string(modelFileName)) == 0 && isprop(obj.model, 'pathModel')
            ...
            semanticConfig = ...
                openmebius.domain.batch.BatchIdentity.semanticConfig(obj.config);
            defaultContentHash = ...
            ...
            randomState = obj.RandomStateAtStart;

            metadata = struct( ...
                'schemaVersion', ...
                double(obj.provenanceValue(provenance, 'schemaVersion', 1)), ...
                'batchId', ...
                string(obj.provenanceValue( ...
                provenance, 'batchId', obj.HDF5FileName)), ...
                'contentHash', ...
                string(obj.provenanceValue( ...
                provenance, 'contentHash', defaultContentHash)), ...
                'contentHashVersion', ...
                double(obj.provenanceValue( ...
                provenance, ...
                'contentHashVersion', ...
                ...
                'startedAtUtc', obj.RunStartedAtUtc);

        end % buildAnalysisMetadata
