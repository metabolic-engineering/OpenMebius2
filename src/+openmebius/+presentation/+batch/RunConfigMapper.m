classdef RunConfigMapper
    ...
    methods (Static, Access = private)
        ...
        function tableData = effluxToTable(config)
            ...
            if isempty(selection) && isempty(substrates) && ...
                    isempty(standardDeviations)
                tableData = table( ...
                    'Size', [0, 2], ...
        ...
        function config = effluxFromTable(tableData)
            ...
            openmebius.presentation.batch.RunConfigMapper ...
                .requireTableVariables( ...
                tableData, ["Selection", "SD"], "Efflux");
            substrates = string(tableData.Properties.RowNames(:));

            if numel(substrates) ~= height(tableData) || ...
                    any(strlength(substrates) == 0)
                openmebius.presentation.batch.RunConfigMapper ...
                    .invalidTable( ...
                    "Efflux table requires one substrate row name " + ...
                "for each row.");
            end

            config = struct( ...
                selection = logical(tableData.Selection(:)), ...
                substrate = substrates, ...
                substrateSD = double(tableData.SD(:)));
